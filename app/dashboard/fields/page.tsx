...
/**
 * Fields page for maintaining the shared extraction template.
 */
    ...
    <>
      <Topbar
        ...
        actions={
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => setDraft(data)} disabled={!isDirty}>
              重置
            </Button>
            <Button onClick={() => updateFields.mutate(draft)} disabled={!isDirty || updateFields.isPending}>
      ...
      <div className="flex flex-1 flex-col gap-4 overflow-y-auto p-5">
        <section className="grid grid-cols-1 gap-3 md:grid-cols-3">
          ...
          <StatCard label="可选扩展字段" value={String(disabledCount)} change="按需开启" trend="neutral" />
          <StatCard label="模板状态" value={isDirty ? '未保存' : '已同步'} change="字段变更实时生效" trend={isDirty ? 'down' : 'up'} />
        </section>
