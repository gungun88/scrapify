...
const navSections: Array<{ label: string; items: NavItem[] }> = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: '价格监控', href: '/dashboard/monitor', icon: Radar, badge: '3' },
  ...
  return (
    <aside className="z-10 flex h-full w-sidebar shrink-0 flex-col border-r border-border bg-surface">
            ...
            {section.items.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {item.pulse ? (
                    <span className="ml-auto h-[7px] w-[7px] rounded-full bg-green animate-pulse" />
                  ) : null}
                </Link>
              )
            })}
      ...
      <div className="border-t border-border px-2 pb-[14px] pt-[10px]">
        <div className="flex cursor-default items-center gap-[9px] rounded-sm px-[10px] py-2 transition-colors hover:bg-surface2">
          <div className="flex h-[30px] w-[30px] items-center justify-center rounded-full bg-brand text-[11px] font-semibold text-white">
            张
          </div>
          <div>
            <div className="text-[13px] font-medium text-text1">张晓晴</div>
            <div className="text-[11px] text-text3">Pro · 剩余 18 天</div>
          </div>
        </div>
      </div>
    </aside>
